...
```sh
docker pull denoland/deno:2.8.2        # Docker Hub
docker pull ghcr.io/denoland/deno:2.8.2 # GHCR
```
...
```sh
$ docker run -it denoland/deno:2.8.2 repl
```

To shell into the docker runtime:

```sh
$ docker run -it denoland/deno:2.8.2 sh
```

To run `main.ts` from your working directory:

```sh
$ docker run -it -p 1993:1993 -v $PWD:/app denoland/deno:2.8.2 run --allow-net /app/main.ts
```
