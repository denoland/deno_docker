...
```sh
$ docker run -it denoland/deno:2.7.1 repl
```

To shell into the docker runtime:

```sh
$ docker run -it denoland/deno:2.7.1 sh
```

To run `main.ts` from your working directory:

```sh
$ docker run -it -p 1993:1993 -v $PWD:/app denoland/deno:2.7.1 run --allow-net /app/main.ts
```
