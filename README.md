...
```sh
$ docker run -it --init hayd/alpine-deno:1.3.2 repl
```

To shell into the docker runtime:

```sh
$ docker run -it --init --entrypoint sh hayd/alpine-deno:1.3.2
```

To run `main.ts` from your working directory:

```sh
$ docker run -it --init -p 1993:1993 -v $PWD:/app hayd/alpine-deno:1.3.2 run --allow-net /app/main.ts
```
