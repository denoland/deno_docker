...
```sh
$ docker run -it denoland/deno:2.2.1 repl
```

To shell into the docker runtime:

```sh
$ docker run -it denoland/deno:2.2.1 sh
```

To run `main.ts` from your working directory:

```sh
$ docker run -it -p 1993:1993 -v $PWD:/app denoland/deno:2.2.1 run --allow-net /app/main.ts
```
...
```Dockerfile
FROM ubuntu
COPY --from=denoland/deno:bin-2.2.1 /deno /usr/local/bin/deno
```
