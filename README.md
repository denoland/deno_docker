...
```sh
$ docker run -it --init denoland/deno:1.11.3 repl
```

To shell into the docker runtime:

```sh
$ docker run -it --init --entrypoint sh denoland/deno:1.11.3
```

To run `main.ts` from your working directory:

```sh
$ docker run -it --init -p 1993:1993 -v $PWD:/app denoland/deno:1.11.3 run --allow-net /app/main.ts
```
