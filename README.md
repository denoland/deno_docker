...
```sh
$ docker run -it denoland/deno:1.46.0-rc.1 repl
```

To shell into the docker runtime:

```sh
$ docker run -it denoland/deno:1.46.0-rc.1 sh
```

To run `main.ts` from your working directory:

```sh
$ docker run -it -p 1993:1993 -v $PWD:/app denoland/deno:1.46.0-rc.1 run --allow-net /app/main.ts
```
...
```Dockerfile
FROM denoland/deno:1.46.0-rc.1
...
```Dockerfile
FROM ubuntu
COPY --from=denoland/deno:bin-1.46.0-rc.1 /deno /usr/local/bin/deno
```
