...
```sh
docker pull denoland/deno:2.9.0        # Docker Hub
docker pull ghcr.io/denoland/deno:2.9.0 # GHCR
```
...
```sh
$ docker run -it denoland/deno:2.9.0 repl
```

To shell into the docker runtime:

```sh
$ docker run -it denoland/deno:2.9.0 sh
```

To run `main.ts` from your working directory:

```sh
$ docker run -it -p 1993:1993 -v $PWD:/app denoland/deno:2.9.0 run --allow-net /app/main.ts
```
