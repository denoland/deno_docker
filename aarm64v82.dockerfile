...
# Build GN for alpine.
#
FROM arm64v8/alpine:3.10.1 as gn-builder
...
RUN sed -i -e 's/v[[:digit:]]\..*\//edge\//g' /etc/apk/repositories && echo 1
RUN apk upgrade --update-cache --available

RUN \
  apk add --no-cache --virtual .gn-build-dependencies \
    alpine-sdk \
    binutils-gold \
    clang \
    curl \
...
#
FROM arm64v8/alpine:3.10.1 as deno-builder
...
#ENV LD_LIBRARY_PATH=/tmp/clang-llvm/lib:$LD_LIBRARY_PATH

RUN rm /deno/third_party/prebuilt/linux64/sccache

WORKDIR deno/cli
RUN RUST_BACKTRACE=1 DENO_NO_BINARY_DOWNLOAD=1 DENO_BUILD_ARGS='clang_use_chrome_plugins=false treat_warnings_as_errors=false use_sysroot=false use_glib=false clang_base_path="/usr" use_gold=true' DENO_GN_PATH=gn cargo install --locked --root .. --path . || echo error
