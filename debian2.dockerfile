...
RUN apt-get install -y gcc llvm lld

WORKDIR /deno/cli
RUN RUST_BACKTRACE=1 DENO_NO_BINARY_DOWNLOAD=1 DENO_BUILD_ARGS='clang_use_chrome_plugins=false treat_warnings_as_errors=false use_sysroot=false clang_base_path="/usr" use_glib=false use_gold=true' DENO_GN_PATH=gn cargo install --locked --root .. --path . --target x86_64-unknown-linux-musl || echo "error"
...
ENV DENO_VERSION=$(deno evaluation console.log(Deno.version.deno))
